...
def run_experiment(dataset_dir, csv_file, num_runs=30):
    print(f"Running LightGBM on {dataset_dir}/{csv_file} for {num_runs} times...")
    ...
    log_dir = os.path.join("log", date_dir, "lightgbm")
    os.makedirs(log_dir, exist_ok=True)
    log_file = os.path.join(log_dir, f"lgbm_{dataset_dir}_{csv_file}_{timestamp}.txt")
...
def main():
    parser = argparse.ArgumentParser(description='Train a LightGBM model on a dataset.')
